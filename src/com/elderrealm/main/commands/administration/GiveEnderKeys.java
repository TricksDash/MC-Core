...
public class GiveEnderKeys implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		for (loopAmount  = 0; loopAmount < Integer.parseInt(args[1]); loopAmount++) {
			target.getInventory().addItem(CrateKeys.enderKey());

		}
		target.sendMessage("");
		target.sendMessage(C.green + "Thank you for supporting the server!\n" + C.yellow + args[1] + C.dpurple
				+ " Ender Keys " + C.green +  "have been added to your inventory!");
		target.sendMessage("");
