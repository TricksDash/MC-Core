...
public class CratesTeleport implements CommandExecutor {
	
	private ElderRealmCore plugin;

	public CratesTeleport(ElderRealmCore pl) {
		plugin = pl;
		...
		cooldownTask.put(player, new BukkitRunnable() {
			public void run() {
				...
				if (cooldownTime.get(player) == 0) {
					...
					Location loc = new Location(Bukkit.getServer().getWorld("ElderRealm"), 285, 71, 734);
