...
public class WildernessCommand implements CommandExecutor {
	...
	public WildernessCommand(ElderRealmCore pl) {
		plugin = pl;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		player.sendMessage(C.gray + "[" + C.red + "Teleport" + C.gray + "] " + C.gray + "Teleporting to " + C.yellow
				+ "Wilderness" + C.gray + " in 5 seconds");
		...
		cooldownTask.put(player, new BukkitRunnable() {
			public void run() {
				...
				if (cooldownTime.get(player) == 0) {
					...
					Location loc = new Location(Bukkit.getServer().getWorld("ElderRealm"), 197, 73, 734);

					loc.setYaw(90);
					player.teleport(loc);
					player.sendMessage(C.gray + "[" + C.red + "Teleport" + C.gray + "] " + C.gray + "Welcome to " + C.dgreen
							+ "Wilderness");
